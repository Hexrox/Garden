...
router.post('/',
  ...
  (req, res) => {
    ...
    const { name, display_name, days_to_harvest, range_min, range_max, notes } = req.body;

    db.run(
      `INSERT INTO plants (user_id, name, display_name, days_to_harvest, range_min, range_max, notes, is_custom)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
      [req.user.id, name.toLowerCase(), display_name || name, days_to_harvest, range_min, range_max, notes],
      function (err) {
        if (err) {
